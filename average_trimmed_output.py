...
filename_output_csv = "averaged_output_NaN_Removed/"+filename[15:]
...
values = [Decimal(0)] * 167
placeHolder = [Decimal(0)] * 167
count = Decimal(0)
video_name = "null"
...
with open(filename, 'r') as inp, open(filename_output_csv, 'a') as out:
    ...
    for row in csv.reader(inp):
        ...
        if video_name != row[165] and count!= Decimal(0):
            
            print("evaluating chunk : " + video_name)
            values=average_values(values,count)
            writer.writerow(values)
            video_name = row[165]
            values = [Decimal(0)] * 167
            values[165] = video_name
            values[166] = row[166]
            count= Decimal(0)

        for i in range(len(row)-2): # -2 -> index 165 and 166 are video_name and class attributes
            try:
                if row[i] != "NaN":
                    values[i] += Decimal(row[i])
                    placeHolder[i] = Decimal(row[i])
                else:
                    values[i] += placeHolder[i]
            except TypeError:
                    print ("Error -- ({}) Column({}) could not be converted to numpy.float!".format(row[i], placeHolder[i]) )
        count = count +Decimal(1)
        video_name= row[165]
        values[165]= row[165]   
        values[166] = row[166]
    inp.close()
with open(filename_output_csv, 'a') as out:
    writer = csv.writer(out)
    print("evaluating chunk : " + video_name)
    values=average_values(values,count)
    writer.writerow(values)    
